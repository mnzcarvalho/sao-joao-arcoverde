...
export const Route = createFileRoute("/polo/$id")({
  ...
  notFoundComponent: () => (
    <div className="p-6 text-center"><p>Polo não encontrado.</p><Link to="/polos" className="text-primary underline">Ver polos</Link></div>
  ),
});
...
function PoloDetail() {
  ...
  const polo = data.polos.find(p => p.id === id);
  if (!polo) throw notFound();
  const shows = data.programacao.filter(s => s.polo === id);
      ...
      <div className="space-y-4 px-4">
        <div className="card-tile p-4">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <p className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 text-accent" /> {polo.endereco}</p>
            <p className="flex items-start gap-2"><Clock className="mt-0.5 h-4 w-4 text-accent" /> {polo.horario}</p>
          </div>
          <a
            href={mapsUrl}
            target="_blank"
            ...
            {shows.map(s => (
              <li key={s.id} className="card-tile flex items-center gap-3 p-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{new Date(s.data + "T12:00:00").toLocaleDateString("pt-BR")} · {s.hora} · {s.genero}</p>
                </div>
              </li>
            ))}
          </ul>
