...
import { useShowsFavoritos, toggleFavorito } from "@/features/favoritos/hooks/useFavoritos";
    ...
    <PageShell>
      ...
      {shows.length === 0 ? (
        <div className="px-6 py-12 text-center">
          ...
          <Link
            to="/programacao"
            className="mt-4 inline-block rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground"
          >
            Ver programação
          </Link>
        </div>
        ...
        <ul className="space-y-3 px-4">
          {shows.map((s) => (
            <li key={s.id} className="card-tile flex items-center gap-3 p-4">
              ...
              <button
                onClick={() => toggleFavorito(s.id, "show")}
