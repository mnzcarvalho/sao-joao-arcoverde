...
export const Route = createFileRoute("/polos")({
  component: Polos,
  head: () => ({ meta: [{ title: "Polos — São João de Arcoverde" }, { name: "description", content: "Todos os polos do São João de Arcoverde com endereços e horários." }] }),
});
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <div className="mt-3 space-y-1 text-xs">
                    <p className="inline-flex items-center gap-1 text-accent"><MapPin className="h-3 w-3" /> {p.endereco}</p>
                    <p className="flex items-center gap-1 text-muted-foreground"><Clock className="h-3 w-3" /> {p.horario}</p>
                  </div>
                </div>
